...
inline void zipMerge(int * & buffer1, int * & zipBuffer, int & buffer1size, int * & buffer2, int & buffer2Size)
{
    ...
    for (int i = 0; i < QUERY_COUNT; ++i)
    {
        ...
        {
            while (tpIndex < buffer2Size)
            {
                zipBuffer[i++] = buffer2[tpIndex++];
            }

            while (zipIndex < buffer1size)
            {
...
struct NodeContainer
{
    ...
    inline void processSolvedPartition2(NodePartition * nodepartition, int * & outPoints, int & outPointsBufferSize, int * & zipBuffer)
    {
        if (outPointsBufferSize >= QUERY_COUNT)
        {
            // We have at least 1 point better than the worst.
    ...
    inline void processPartialPartition(const Rect & rect, int partition_index, int * & outPoints, int & outPointsBufferSize, int * & zipBuffer, int (&pointBuffer)[QUERY_COUNT])
    {
        NodePartition * nodepartition = &partitions[partition_index];

        int worstRank = (outPointsBufferSize == QUERY_COUNT) ? PointCopy[outPoints[outPointsBufferSize - 1]].rank : INT_MAX;
        ...
        //if (nodepartition->extraPartitions != NULL)
        //{
        //    if (nodepartition->extraPartitions[0].PointsSize)
        //    {
        //        if (containsRect(rect, nodepartition->extraPartitions[0].rect))
        //        {
        //            processSolvedPartition2(&nodepartition->extraPartitions[0], outPoints, outPointsBufferSize, zipBuffer);
        //        }
        //        else if (intersects(rect, nodepartition->extraPartitions[0].rect))
        //        {
        //            processPartialPartition2(rect, &nodepartition->extraPartitions[0], outPoints, outPointsBufferSize, zipBuffer, pointBuffer);
        //        }
        //    }
        //
        //    if (nodepartition->extraPartitions[1].PointsSize > 0)
        //    {
        //        if (containsRect(rect, nodepartition->extraPartitions[1].rect))
        //        {
        //            processSolvedPartition2(&nodepartition->extraPartitions[1], outPoints, outPointsBufferSize, zipBuffer);
        //        }
        //        else if (intersects(rect, nodepartition->extraPartitions[1].rect))
        //        {
        //            processPartialPartition2(rect, &nodepartition->extraPartitions[1], outPoints, outPointsBufferSize, zipBuffer, pointBuffer);
        //        }
        //    }
        //
        //    if (nodepartition->extraPartitions[2].PointsSize > 0)
        //    {
        //        if (containsRect(rect, nodepartition->extraPartitions[2].rect))
        //        {
        //            processSolvedPartition2(&nodepartition->extraPartitions[2], outPoints, outPointsBufferSize, zipBuffer);
        //        }
        //        else if (intersects(rect, nodepartition->extraPartitions[2].rect))
        //        {
        //            processPartialPartition2(rect, &nodepartition->extraPartitions[2], outPoints, outPointsBufferSize, zipBuffer, pointBuffer);
        //        }
        //    }
        //
        //    if (nodepartition->extraPartitions[3].PointsSize > 0)
        //    {
        //        if (containsRect(rect, nodepartition->extraPartitions[3].rect))
        //        {
        //            processSolvedPartition2(&nodepartition->extraPartitions[3], outPoints, outPointsBufferSize, zipBuffer);
        //        }
        //        else if (intersects(rect, nodepartition->extraPartitions[3].rect))
        //        {
        //            processPartialPartition2(rect, &nodepartition->extraPartitions[3], outPoints, outPointsBufferSize, zipBuffer, pointBuffer);
        //        }
        //    }
        //}
        //else 
...
//
//    inline void processPartialPartition2(const Rect & rect, NodePartition * nodepartition, int * & outPoints, int & outPointsBufferSize, int * & zipBuffer, int * pointBuffer)
//    {
//        int worstRank = (outPointsBufferSize == QUERY_COUNT) ? PointCopy[outPoints[outPointsBufferSize - 1]].rank : INT_MAX;
//
//        int pointBufferSize = 0;
...
//            }
//        }
//        else if (outPointsBufferSize == QUERY_COUNT)
//        {
//#ifdef __ANALYTICS__
//            depthCounts[depth] += 1;
//            deeperCount++;
...
//    }
    ...
    inline void processUnsolvedPartial(const Rect & rect, int partition_index, int * & outPoints, int & outPointsBufferSize, int * & zipBuffer, int (&pointBuffer)[QUERY_COUNT])
    {
    ...
    void AddPoints(Point * points, int PointCopySize)
    {
        ...
        // Extra Points for those pesky bottom nodes.
        for (int i = 0; i < InitialPoints.size(); ++i)
        {
            if (InitialPoints[i].size() > XY_PARTIAL_THRESHOLD)
            {
                AddMoarPoints(InitialPoints[i], &partitions[i]);
            }
        }
    }

    void AddMoarPoints(vector<int> & points, NodePartition * partition)
    {
//        partition->extraPartitions = new NodePartition[4];
//
//        float lowx = partition->rect.lx;
//        float midx = (partition->rect.lx + partition->rect.hx) / 2.0f;
//        float highx = partition->rect.hx;
//        float lowy = partition->rect.ly;
//        float midy = (partition->rect.ly + partition->rect.hy) / 2.0f;
//        float highy = partition->rect.hy;
//        partition->extraPartitions[0].rect = { lowx, lowy, midx, midy };
//        partition->extraPartitions[1].rect = { lowx, midy, midx, highy };
//        partition->extraPartitions[2].rect = { midx, lowy, highx, midy };
//        partition->extraPartitions[3].rect = { midx, midy, highx, highy };
//
//        vector<int> rect0_points;
//        vector<int> rect1_points;
//        vector<int> rect2_points;
//        vector<int> rect3_points;
//
//        for (int point_index = 0; point_index < points.size(); ++point_index)
//        {
//            Point * point = &PointCopy[points[point_index]];
//            // Use else-ifs to guarantee uniqueness on the boundaries.
//            if (isPointInsideRect(partition->extraPartitions[0].rect, point->x, point->y))
//            {
//                rect0_points.push_back(points[point_index]);
//            }
//            else if (isPointInsideRect(partition->extraPartitions[1].rect, point->x, point->y))
//            {
//                rect1_points.push_back(points[point_index]);
//            }
//            else if (isPointInsideRect(partition->extraPartitions[2].rect, point->x, point->y))
//            {
//                rect2_points.push_back(points[point_index]);
//            }
//            else if (isPointInsideRect(partition->extraPartitions[3].rect, point->x, point->y))
//            {
//                rect3_points.push_back(points[point_index]);
//            }
//        }
//
//        if (rect0_points.size())
//        {
//            partition->extraPartitions[0].AddPoints(rect0_points, PointCopy[rect0_points[0]].rank);
//            if (rect0_points.size() != points.size() && rect0_points.size() > XY_PARTIAL_THRESHOLD)
//            {
//                AddMoarPoints(rect0_points, &partition->extraPartitions[0]);
//            }
//        }
//        if (rect1_points.size())
//        {
//            partition->extraPartitions[1].AddPoints(rect1_points, PointCopy[rect1_points[0]].rank);
//            if (rect1_points.size() != points.size() && rect1_points.size() > XY_PARTIAL_THRESHOLD)
//            {
//                AddMoarPoints(rect1_points, &partition->extraPartitions[1]);
//            }
//        }
//        if (rect2_points.size())
//        {
//            partition->extraPartitions[2].AddPoints(rect2_points, PointCopy[rect2_points[0]].rank);
//            if (rect2_points.size() != points.size() && rect2_points.size() > XY_PARTIAL_THRESHOLD)
//            {
//                AddMoarPoints(rect2_points, &partition->extraPartitions[2]);
//            }
//        }
//        if (rect3_points.size())
//        {
//            partition->extraPartitions[3].AddPoints(rect3_points, PointCopy[rect3_points[0]].rank);
//            if (rect3_points.size() != points.size() && rect3_points.size() > XY_PARTIAL_THRESHOLD)
//            {
//                AddMoarPoints(rect3_points, &partition->extraPartitions[3]);
//            }
//        }
    }
};
...
PointContainer::PointContainer(const Point * points_begin, const Point * points_end)
{
    ...
    //yxPointsASize = 0;
    //yxPointsBSize = 0;
    //float midx = (mainRect.hx + mainRect.lx) / 2.0f;
    //float midy = (mainRect.hy + mainRect.ly) / 2.0f;
    //for (int point = 0; point < xyPointsSize; point++)
    //{
    //    if (PointCopy[xyPoints[point]].y <= midy)
    //    {
    //        xyPointsASize++;
    //    }
    //    // Need to handle both sides so items in the boundary are included.
    //    if (PointCopy[xyPoints[point]].y >= midy)
    //    {
    //        xyPointsBSize++;
    //    }
    //}
    //for (int point = 0; point < yxPointsSize; point++)
    //{
    //    if (PointCopy[yxPoints[point]].x <= midx)
    //    {
    //        yxPointsASize++;
    //    }
    //    // Need to handle both sides so items in the boundary are included.
    ...
    //    }
    //}
    //xyPointsA = new int[xyPointsASize];
    //xyPointsB = new int[xyPointsBSize];
    //yxPointsA = new int[yxPointsASize];
    //yxPointsB = new int[yxPointsBSize];
    //
    //xyPointsASize = 0;
    //xyPointsBSize = 0;
    //yxPointsASize = 0;
    //yxPointsBSize = 0;
    //
    //xyPointsARect = { mainRect.lx, mainRect.ly, mainRect.hx, midy };
    //xyPointsBRect = { mainRect.lx, midy, mainRect.hx, mainRect.hy };
    //yxPointsARect = { mainRect.lx, mainRect.ly, midx, mainRect.hy };
    //yxPointsBRect = { midx, mainRect.ly, mainRect.hx, mainRect.hy };
    //
    //for (int point = 0; point < xyPointsSize; point++)
    //{
    //    if (PointCopy[xyPoints[point]].y <= midy)
    //    {
    //        xyPointsA[xyPointsASize++] = xyPoints[point];
    //    }
    //    // Need to handle both sides so items in the boundary are included.
    //    if (PointCopy[xyPoints[point]].y >= midy)
    //    {
    //        xyPointsB[xyPointsBSize++] = xyPoints[point];
    //    }
    //}
    //for (int point = 0; point < yxPointsSize; point++)
    //{
    //    if (PointCopy[yxPoints[point]].x <= midx)
    //    {
    //        yxPointsA[yxPointsASize++] = yxPoints[point];
    //    }
    //    // Need to handle both sides so items in the boundary are included.
    //    if (PointCopy[yxPoints[point]].x >= midx)
    //    {
    //        yxPointsB[yxPointsBSize++] = yxPoints[point];
    //    }
    //}
    ...
    if (PointCopySize)
    {
        ...
        yMult = nodeContainer[MAX_DEPTH - MIN_DEPTH].ymult;
        xMult = nodeContainer[MAX_DEPTH - MIN_DEPTH].xmult;
    }
    BASELINE_FACTOR = 1.3f * pow(0.5f, MIN_DEPTH); // Magic number
...
int getXYPoints(const Rect & rect, int & lowIndex, int & highIndex, int * & xyPoints, int & xyPointsSize)
{
    int lowPoint = 0;
...
void solveXYRect(const Rect & rect, int solvedLowPoint, int solvedHighPoint, int * & xyPoints)
{
    t1_outPointsBufferSize = 0;
...
int getYXPoints(const Rect & rect, int & lowIndex, int & highIndex, int * & yxPoints, int & yxPointsSize)
{
    int lowPoint = 0;
...
void solveYXRect(const Rect & rect, int solvedLowPoint, int solvedHighPoint, int * & yxPoints)
{
    t1_outPointsBufferSize = 0;
    int * pointBuffer = &t1_pointBuffer[0];
    int pointBufferSize = 0;
...
int getXYPointsPartition(const Rect & rect, int & lowIndex, int & highIndex, int * & xyPoints, int & xyPointsSize)
{
    int lowPoint = 0;
    int highPoint = xyPointsSize - 1;
...
inline void solveXYRectPartition(const Rect & rect, int * & xyPoints, int & xyPointsSize, int * & outPoints, int & outPointsSize, int * & pointBuffer, int * & zipBuffer, int worstRank)
{
    int pointBufferSize = 0;
    int solvedLowPoint;
    int solvedHighPoint;

    getXYPointsPartition(rect, solvedLowPoint, solvedHighPoint, xyPoints, xyPointsSize);
    ...
    while (++solvedLowPoint <= solvedHighPoint)
    {
        if (PointCopy[xyPoints[solvedLowPoint]].y >= rect.ly && PointCopy[xyPoints[solvedLowPoint]].y <= rect.hy)
        {
            if (PointCopy[xyPoints[solvedLowPoint]].rank < worstRank)
            {
                ...
                {
                    std::sort(pointBuffer, pointBuffer + QUERY_COUNT, pointCopyRankComparison);
                    zipMerge(outPoints, zipBuffer, outPointsSize, pointBuffer, pointBufferSize);
                    worstRank = outPoints[outPointsSize - 1];
                    pointBufferSize = 0;
                }
            }
        }
...
int getYXPointsPartition(const Rect & rect, int & lowIndex, int & highIndex, int * & yxPoints, int & yxPointsSize)
{
    int lowPoint = 0;
    int highPoint = yxPointsSize - 1;
...
inline void solveYXRectPartition(const Rect & rect, int * & yxPoints, int & yxPointsSize, int * & outPoints, int & outPointsSize, int * & pointBuffer, int * & zipBuffer, int worstRank)
{
    int pointBufferSize = 0;
    int solvedLowPoint;
    int solvedHighPoint;

    getYXPointsPartition(rect, solvedLowPoint, solvedHighPoint, yxPoints, yxPointsSize);
...
int32_t PointContainer::Evaluate(const Rect rect, const int32_t count, Point* out_points)
{
    ...
    if (containsRect(rect, mainRect))
    {
        found = (int32_t)top20.size();
        copy(top20.begin(), top20.begin() + top20.size(), out_points);
    }
    else if (intersects(rect, mainRect))
    {
        // Guess how deep we should initially go.
        Rect pruned = pruneRect(rect, mainRect);
        float width = pruned.hx - pruned.lx;
        float height = pruned.hy - pruned.ly;

        bool lowX = width <= xMult;
        bool lowY = height <= yMult;

        if (lowX && lowY)
        {
            // Deep dive to the lowest container.
            nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
        }
        else if (10 * width <= xMult)
        {
            int solvedLowPoint, solvedHighPoint;
            /*if (containsRect(xyPointsARect, rect))
            {
                if (getXYPoints(rect, solvedLowPoint, solvedHighPoint, xyPointsA, xyPointsASize) < XY_THRESHOLD)
                {
                    solveXYRect(pruned, solvedLowPoint, solvedHighPoint, xyPointsA);
                }
                else
                {
                    nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
            ...
            else if (containsRect(xyPointsBRect, rect))
            {
                if (getXYPoints(rect, solvedLowPoint, solvedHighPoint, xyPointsB, xyPointsBSize) < XY_THRESHOLD)
                {
                    solveXYRect(pruned, solvedLowPoint, solvedHighPoint, xyPointsB);
                }
                else
                {
                    nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
            ...
            else */if (getXYPoints(rect, solvedLowPoint, solvedHighPoint, xyPoints, xyPointsSize) < XY_THRESHOLD)
            {
                solveXYRect(pruned, solvedLowPoint, solvedHighPoint, xyPoints);
            }
            else
            {
                nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
        ...
        else if (10 * height <= yMult)
        {
            int solvedLowPoint, solvedHighPoint;
            /*if (containsRect(yxPointsARect, rect))
            {
                if (getYXPoints(rect, solvedLowPoint, solvedHighPoint, yxPointsA, yxPointsASize) < XY_THRESHOLD)
                {
                    solveYXRect(pruned, solvedLowPoint, solvedHighPoint, yxPointsA);
                }
                else
                {
                    nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
            ...
            else if (containsRect(yxPointsBRect, rect))
            {
                if (getYXPoints(rect, solvedLowPoint, solvedHighPoint, yxPointsB, yxPointsBSize) < XY_THRESHOLD)
                {
                    solveYXRect(pruned, solvedLowPoint, solvedHighPoint, yxPointsB);
                }
                else
                {
                    nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
            ...
            else */if (getYXPoints(rect, solvedLowPoint, solvedHighPoint, yxPoints, yxPointsSize) < XY_THRESHOLD)
            {
                solveYXRect(pruned, solvedLowPoint, solvedHighPoint, yxPoints);
            }
            else
            {
                nodeContainer[MAX_DEPTH - MIN_DEPTH].Calculate(pruned, count, false);
...
unsigned int __stdcall t2Thread(void* data)
{
    while (!exit_t2)
    {
        if (run_t2_partial)
        {
            ...
            {
                t2_NodeContainer->processPartialPartition(t2_rect, partial[i], t2_outPoints, t2_outPointsBufferSize, t2_zipBuffer, t2_pointBuffer);
            }
...
void PointContainer::StartWorkerThreads()
{
    ...
    SetThreadPriority(t2Handle, THREAD_PRIORITY_TIME_CRITICAL);
