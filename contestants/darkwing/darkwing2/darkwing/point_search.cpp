...
namespace pointSearch
{
	...
	__forceinline bool compare_rank(const Point& first, const Point& second)
	{
		return (first.rank < second.rank);
	}

	// compare x of 2 points
	__forceinline bool compare_x(const Point& first, const Point& second)
	{
	...
	SearchContext::SearchContext(const Point* points_begin, const Point* points_end)
	{
		...
		// get area size
		m_sqare = sqrt(500 * m_pointsToFind / m_frequency);
		if(m_sqare == 0)
			m_sqare = 1;
		...
		for(m_i = 0; m_i < m_xAreas; m_i++)
		{
			for(m_j = 0; m_j < m_yAreas; m_j++)
			{
				for(m_k = 0; m_k < m_rankLayerCount + 1; m_k++)
				{
					std::sort(m_AreaLayer[m_i][m_j][m_k]->begin(), m_AreaLayer[m_i][m_j][m_k]->end(), compare_x);
				}
			}
		}
		m_i = 0;
		m_pointsInRect.reserve(m_pointCount);
		m_sizeOfPoint = sizeof(Point);
	}
	...
	SearchContext::search(const Rect rect, const int32_t count, Point* out_points)
	{
		...
		if(m_lastAreaY < 1)
			m_lastAreaY = 0;
		m_rankLayer = 0;
		m_pointsInRect.resize(0);
		
		while ((m_pointsInRect.size() < 20) && (m_rankLayer < m_rankLayerCount))
		{
			...
			if (m_firstAreaX != m_lastAreaX)
			{
				...
				if (m_leftParted)
				{
					for(m_y = m_firstAreaY + 1; m_y < m_lastAreaY; m_y++)
					{
						m_j = m_AreaLayer[m_firstAreaX][m_y][m_rankLayer]->size() - 1;
						for (int m_i = m_j; m_i > -1; m_i--)
						{
				...
				for(m_y = m_firstAreaY + 1; m_y < m_lastAreaY; m_y++)
				{
					m_j = m_AreaLayer[m_lastAreaX][m_y][m_rankLayer]->size();
					for (int m_i = 0; m_i < m_j; m_i++)
					{
			...
			else
			{
				...
				for(m_y = m_firstAreaY + 1; m_y < m_lastAreaY; m_y++)
				{
					m_j =  m_AreaLayer[m_firstAreaX][m_y][m_rankLayer]->size();
					for (int m_i = 0; m_i < m_j; m_i++)
					{
			...
			if (m_firstAreaY != m_lastAreaY)
			{
				...
				if(m_bottomParted)
				{
					for(m_x = m_firstAreaX + 1; m_x < m_lastAreaX; m_x++)
					{
						m_j = m_AreaLayer[m_x][m_firstAreaY][m_rankLayer]->size();
						for (int m_i = 0; m_i < m_j; m_i++)
						{
				...
				for(m_x = m_firstAreaX + 1; m_x < m_lastAreaX; m_x++)
				{
					m_j = m_AreaLayer[m_x][m_lastAreaY][m_rankLayer]->size();
					for (int m_i = 0; m_i < m_j; m_i++)
					{
			...
			if(m_firstAreaX == m_lastAreaX)
			{
				...
				{
					// just 1 corner area
					m_j = m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->size();
					for (m_i = 0; m_i < m_j; m_i++)
					{

						if (m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->at(m_i).x <= rect.hx)
						{
				...
				else
				{
					...
					// top
					m_j = m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->size();
					for (m_i = 0; m_i < m_j; m_i++)
					{
						if (m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->at(m_i).x <= rect.hx)
						{
			...
			else
			{
				if (m_firstAreaY == m_lastAreaY)
				{
					...
					// left
					m_j = m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->size() - 1;
					for (m_i = m_j; m_i > -1; m_i--)
					{
						if (m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->at(m_i).x >= rect.lx)
						{
							if ((m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->at(m_i).y <= rect.hy) &&
								(m_AreaLayer[m_firstAreaX][m_firstAreaY][m_rankLayer]->at(m_i).y >= rect.ly))
							{
					...
					// rigth
					m_j = m_AreaLayer[m_lastAreaX][m_firstAreaY][m_rankLayer]->size();
					for (m_i = 0; m_i < m_j; m_i++)
					{
						if (m_AreaLayer[m_lastAreaX][m_firstAreaY][m_rankLayer]->at(m_i).x <= rect.hx)
						{
				...
				{
					// upper left corner
					m_j = m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->size() - 1;
					for (m_i = m_j; m_i > -1; m_i--)
					{
						if (m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->at(m_i).x >= rect.lx)
						{
							if (m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->at(m_i).y <= rect.hy)
							{
								m_pointsInRect.push_back(m_AreaLayer[m_firstAreaX][m_lastAreaY][m_rankLayer]->at(m_i));
					...
					// upper right corner
					m_j = m_AreaLayer[m_lastAreaX][m_lastAreaY][m_rankLayer]->size();
					for (m_i = 0; m_i < m_j; m_i++)
					{
						if (m_AreaLayer[m_lastAreaX][m_lastAreaY][m_rankLayer]->at(m_i).x <= rect.hx)
						{
		...
		m_outPointsCount = m_pointsInRect.size();
		if(m_outPointsCount > 20)
			m_outPointsCount = 20;
		memcpy(out_points, &m_pointsInRect.front(), m_outPointsCount * m_sizeOfPoint);
