...
// 
// struct CmpPointRankHeap {
//     inline bool operator()(const PtAligned& p1, const PtAligned& p2) const 
//     {
//         return p1.F >= p2.F;
//     }
// };
...
class ResultVector
{
    typedef vector<PtAligned> TList;
...
public:
    ...
    ResultVector(int nSize)
    {
        pts.insert(pts.end(), nSize, PtAligned(0));
        front = &pts[0];
        back = front + nSize;
        --back;
    }
    ...
    {
        if(back->rank > pt.rank)
        {
            PtAligned *ptWhere = lower_bound(front, back+1, pt, CmpPointRank());
    ...
    // Get the results out, we need to re-sort here in the rare case that pts.size() < nSize at the end of processing
    int getPts(Point *ptOut)
    {
        sort(ALL(pts), CmpPointRank());
        int n = 0;
        FOR_EACH(PtIter, pi, pts)
        {
            if(pi->writeToPointPacked(ptOut)) 
            {
                ++ptOut;
                ++n;
            }
...
struct Chunk
{
    ...
    Chunk(PtAligned *beg, PtAligned *end): beg(beg), end(end){}
    //////////////////////////////////////////////////////////////////////////
...
struct SearchContext
{
    ...
    void splitAll()
    {
        ...
        FOR_N(j, SPLITS)
        {
            ...
            FOR_EACH(Chunks::iterator, i, m_Chunks)
            {
                ...
                if(bHasDistinctY)
                {
                    PtAligned* pPtsMid = i->beg + i->size() / 2;
    ...
    void searchOnRects(const Rect &rc, vector<size_t>& vecOnRectIdx, Results &results)
    {
        ...
        FOR_N(i, vecOnRectIdx.size())
        {
            ...
            if(c.beg->rank > results.back->rank) 
            {
                continue;
            }
    ...
    int32_t searchIt(const Rect rec, const int32_t nRequested, Point* pPtsOut)
    {
        ...
        if(m_nPts)
        {
            ...
            {
                fprintf(g_LogFile, "%f %f %d %d\n", pPtsOut->x, pPtsOut->y, pPtsOut->rank, (int)pPtsOut->id);
                ++pPtsOut;
            }
#endif
...
{
// #define PRINT_SIZE(X)  fprintf(stderr, "sizeof" #X " : %d\n", sizeof(X));
// 
//     fprintf(stderr, "\n");
//     PRINT_SIZE(Point);
//     PRINT_SIZE(PtAligned);
//     PRINT_SIZE(Chunk);
//     fprintf(stderr, "\n");
#ifdef TEST
