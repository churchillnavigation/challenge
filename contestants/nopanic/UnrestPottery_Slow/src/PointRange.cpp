	...
	outside_(0)
{
	...
	float mx = (span_.lx + span_.hx) * 0.5;
	float my = (span_.ly + span_.hy) * 0.5;
	float dx = (span_.hx - span_.lx) * 0.5;
	float dy = (span_.hy - span_.ly) * 0.5;
	
	const float SMALL_SPAN = 0.9;
...
const Point& PointRange::operator[](int i) const
{
