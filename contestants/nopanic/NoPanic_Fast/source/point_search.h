...
typedef SearchContext* (*T_create)(const Point* points_begin, const Point* points_end);
...
typedef int32_t (*T_search)(SearchContext* sc, const Rect rect, const int32_t count, Point* out_points);

/* Release the resources associated with the context. Return nullptr if successful, "sc" otherwise. */
typedef SearchContext* (*T_destroy)(SearchContext* sc);
