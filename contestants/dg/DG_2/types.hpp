...
public:
    ...
    // Return whether the bounding box fully contains another bounding box.
    bool contains(bounding_box const& other) const {
        return m_bounds[MIN][X] <= other[MIN][X] && other[MAX][X] <= m_bounds[MAX][X]
            && m_bounds[MIN][Y] <= other[MIN][Y] && other[MAX][Y] <= m_bounds[MAX][Y];
    }
