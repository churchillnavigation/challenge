...
priority_kd_tree::priority_kd_tree(identified_ranked_2d_point* points_begin, identified_ranked_2d_point* points_end)
    : m_nodes(std::distance(points_begin, points_end)), 
      m_caches( std::min<std::size_t>(number_of_nodes_for_levels(levels_with_caches), 
                                      std::distance(points_begin, points_end)) ),
      m_bbox(points_begin, points_end)
{
    if (m_nodes.empty()) {
    ...
    std::size_t index, axis_t split_axis)
{
    ...
    // Except for subtrees near the bottom of the tree, we cache the subset of points with smallest rank in the subtree.
    if (level_of(index) < levels_with_caches) {
        auto& cache = m_caches[index];
        std::partial_sort_copy(points_begin, points_end, cache.begin(), cache.end(), 
            [](identified_ranked_2d_point const& p1, identified_ranked_2d_point const& p2) {
                return p1.rank < p2.rank;
        });
    }

    // Store the minimum rank point in the node, and remove it from the point set.
...
void priority_kd_tree::collect_points_in_rectangle(
    result_queue& results, bounding_box const& query_rectangle, std::size_t index, axis_t split_axis, 
    bounding_box const& subtree_bbox) const
{
    auto& node = m_nodes[index];
    if (query_rectangle.contains(subtree_bbox) && level_of(index) < levels_with_caches) {
        // If the bounding box of the points in this subtree is fully contained in the query rectangle, then obviously all
        // these points are also contained in the query rectangle.  If this happens in one of the levels for which we have 
        // caches, that means we can just retrieve the lowest ranked points in the query rectangle without traversing the
        // tree further.
        for (auto&& point : m_caches[index]) {
            // If the result queue was already full with points of lesser rank then we can stop, because the cached 
            // points are stored in order of rank.
            if (results.filled_with_points_of_smaller_rank_than(point)) {
                break;
            } else {
                results.add(point);
            }
        ...
        return;
    } else if (query_rectangle.contains(node.point)) {
    ...
    if (query_rectangle[MIN][split_axis] <= node.split) {
        collect_points_in_rectangle(results, query_rectangle, left_child(index), next_axis(split_axis), 
                                    subtree_bbox.replace(MAX, split_axis, node.split));
    }
    if (node.split <= query_rectangle[MAX][split_axis]) {
        collect_points_in_rectangle(results, query_rectangle, right_child(index), next_axis(split_axis), 
                                    subtree_bbox.replace(MIN, split_axis, node.split));
    }
}
