...
protected:
    ...
    // Return the total number of nodes for a binary heap with `num_levels` levels, all of which are full.
    static std::size_t number_of_nodes_for_levels(std::size_t num_levels) {
        return (1 << num_levels) - 1; // sum_{i=0}^{n-1} 2^i == 2^n - 1
    }
