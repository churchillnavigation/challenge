...
Solution::Solution(const Point *points_begin, const Point *points_end)
    ...
    ,   m_avx_count(std::min(AVX_COUNT, (point_index)m_points.size()))
{
    if (m_points.empty()) {
        return;
    }
    ...
    m_x_coord.resize(m_avx_count);
    std::transform(begin(m_points), begin(m_points) + m_avx_count, begin(m_x_coord), util::extract_x);
    m_y_coord.resize(m_avx_count);
    std::transform(begin(m_points), begin(m_points) + m_avx_count, begin(m_y_coord), util::extract_y);

    // mipmap data structure
    // skip the indices already covered by linear search
    auto first = begin(m_points) + m_avx_count;
    auto pivot = first;
...
point_index Solution::search_linear(const Rect rect, const point_index count, Point *out_points)
{
    ...
    for(; i < (end - 7); i+=8) {
        ...
        if (!_mm256_testz_ps(x_in, y_in))
        {
            ...
            __m128i upper = _mm_castps_si128(_mm256_extractf128_ps(mask, 1));
            __m128i lower = _mm_castps_si128(_mm256_extractf128_ps(mask, 0));

            if (_mm_extract_epi32(lower, 0)) {out_points[n++] = m_points[i+0]; if (n==count) return count;}
            if (_mm_extract_epi32(lower, 1)) {out_points[n++] = m_points[i+1]; if (n==count) return count;}
            if (_mm_extract_epi32(lower, 2)) {out_points[n++] = m_points[i+2]; if (n==count) return count;}
            if (_mm_extract_epi32(lower, 3)) {out_points[n++] = m_points[i+3]; if (n==count) return count;}
            if (_mm_extract_epi32(upper, 0)) {out_points[n++] = m_points[i+4]; if (n==count) return count;}
            if (_mm_extract_epi32(upper, 1)) {out_points[n++] = m_points[i+5]; if (n==count) return count;}
            if (_mm_extract_epi32(upper, 2)) {out_points[n++] = m_points[i+6]; if (n==count) return count;}
            if (_mm_extract_epi32(upper, 3)) {out_points[n++] = m_points[i+7]; if (n==count) return count;}
        }
    }
                              ...
                              const point_index count, RankHeap& heap)
{
    ...
    for(;i < sse_last; i+=8)
    {
        ...
        if (!_mm256_testz_ps(low_in, high_in))
        {
            __m256 mask = _mm256_and_ps(low_in, high_in);
            __m128i upper = _mm_castps_si128(_mm256_extractf128_ps(mask, 1));
            __m128i lower = _mm_castps_si128(_mm256_extractf128_ps(mask, 0));

            if (_mm_extract_epi32(lower, 0)) {heap.push(indices[i+0]);}
            if (_mm_extract_epi32(lower, 1)) {heap.push(indices[i+1]);}
            if (_mm_extract_epi32(lower, 2)) {heap.push(indices[i+2]);}
            if (_mm_extract_epi32(lower, 3)) {heap.push(indices[i+3]);}
            if (_mm_extract_epi32(upper, 0)) {heap.push(indices[i+4]);}
            if (_mm_extract_epi32(upper, 1)) {heap.push(indices[i+5]);}
            if (_mm_extract_epi32(upper, 2)) {heap.push(indices[i+6]);}
            if (_mm_extract_epi32(upper, 3)) {heap.push(indices[i+7]);}
        }
    }
