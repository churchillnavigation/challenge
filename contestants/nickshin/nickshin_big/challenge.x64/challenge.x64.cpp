...
/* CHANGELIST:
 * v04.1.1: CHALLENGE2 - MEMORY OPTIMIZATIONS
...
void QuadTree::sort()
{
	...
	std::sort(RANKS.begin(), RANKS.end(), compare_points);
...
void QuadTree::GetPoints(Point* out_points, SearchContext* sc, const Rect* range, int32_t count)
{
	...
	// ELSE DRILL DOWN...
	nw->GetPoints(out_points, sc, range, count);
	ne->GetPoints(out_points, sc, range, count);
	sw->GetPoints(out_points, sc, range, count);
	se->GetPoints(out_points, sc, range, count);
}
