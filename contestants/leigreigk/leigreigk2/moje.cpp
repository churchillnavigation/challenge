...
const int maxtree0_size=524280;
for(int nodeIT=0;nodeIT<2;nodeIT++){
TreeNode *Node;
Point *data;
int lenght;
if(nodeIT==0){
    Node=Node0;
    data=point_table;
    lenght=std::min(size,maxtree0_size);
}
if(nodeIT==1){
    Node=Node1;
    data=&(point_table[std::min(size,maxtree0_size)]);
    lenght=std::max(0,size-maxtree0_size);
}
...
if(output_sort_len<=20)ret= Search2D(*Node1,rect,check_bounds/*,out_points*/);
