...
static uint32_t s_cell_size = 72000;
...
public:
    ...
    // ------------------------------------------------------------------------
    void subdivide(Search *ctx, vector<Pt> &points, uint32_t depth, uint32_t keep_count);
...
public:
    typedef QuadTree Qt;
    static const uint32_t _num_qt = 3;
        ...
        _root_qt(nullptr)
    {
        ...
        _root_qt->subdivide(this, points, 0, keep_count);
    }
    ...
    void _search(const Target &target)
    {
        ...
        // ------------------------------------------------------------
        int nth_rank = _root_qt->search(this, _pointset, target, 0, INT_MAX);
                         ...
                         uint32_t keep_count)
{
    ...
    if (num_points <= s_cell_size || supertight(_box))
    {
    ...
    // -------------------------------------------------------
    _storepoints(ctx, points, std::min(num_points, keep_count));
    vector<Pt>().swap(points); 
    ...
    for (uint32_t i=0; i<4; ++i)
        if (_child[i])
            _child[i]->subdivide(ctx, child_points[i], depth+1, keep_count);
}
    ...
    if (_leaf)
    {
        ...
        if (_has_intervals)
        {
            ...
            if (target.small_part_of(_box, along_x))
            {
                ...
                if (num_hits < (nth_rank == INT_MAX ? (uint32_t)(s_max_num_hits * 1.5f) : s_max_num_hits))
                {
                    if (num_hits > 1)
