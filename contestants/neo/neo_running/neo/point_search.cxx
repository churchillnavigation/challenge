...
static uint32_t s_depth_threshold = 6; 
static int      s_num_threads = 2;
...
public:
    ...
    int32_t search(const Rect rect, const int32_t count, Point* out_points)
    {
        ...
        if (s_filter)
        {
            float target_area = target.area(); // for debugging: todo REMOVE

            // process only queries where rect area between s_area_filter_lb and
            // s_area_filter_hb
            // -----------------------------------------------------------------
            if (!(target_area > s_area_filter_lb &&
                  (target_area <= s_area_filter_hb || s_area_filter_hb > 1000000000)))
                return 0;
        }

        // fill in "fully inside" first, and then process the "partial"
        // ------------------------------------------------------------
        int nth_rank = _ot.search(_pointset, target, *this, 0, INT_MAX);
        
        size_t num_partial = _qt_partial.size();
        if (num_partial > 1)
            std::sort(_qt_partial.begin(), _qt_partial.end(), 
                      [](const Qt *a, const Qt *b) -> bool { return a->ia() > b->ia(); });

        for (size_t i=0; i<num_partial; ++i)
            nth_rank = _qt_partial[i]->search_partial(_pointset, target, nth_rank);
                                                                 ...
                                                                 const Point* points_end)
{
    ...
    get_from_environment(s_num_threads,       "num_threads");
