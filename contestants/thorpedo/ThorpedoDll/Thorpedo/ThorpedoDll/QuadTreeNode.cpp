...
QuadTreeNode::QuadTreeNode(uint32_t xmin, uint32_t ymin, uint32_t xmax, uint32_t ymax, int depth) : m_Best(1<<30), m_IsSplit(false),m_IsPruned(false),
...
void QuadTreeNode::AddPointSplit(const QueryPoint& p)
{
	...
	if (NormRect::RectContainsPoint(m_Bounds.m_LX, m_Bounds.m_LY, m_XC, m_YC, p.m_NX, p.m_NY))
	{
		if(m_NLL == nullptr)
		{
			m_NLL = new QuadTreeNode(m_Bounds.m_LX, m_Bounds.m_LY, m_XC, m_YC, m_Depth-1);
		}
		m_NLL->AddPoint(p);
	}
	else if (NormRect::RectContainsPoint(m_Bounds.m_LX, m_YC, m_XC, m_Bounds.m_HY, p.m_NX, p.m_NY))
	{
		if(m_NLH == nullptr)
		{
			m_NLH = new QuadTreeNode(m_Bounds.m_LX, m_YC, m_XC, m_Bounds.m_HY, m_Depth-1);
		}
		m_NLH->AddPoint(p);
	}
	else if (NormRect::RectContainsPoint(m_XC, m_Bounds.m_LY, m_Bounds.m_HX, m_YC, p.m_NX, p.m_NY))
	{
		if(m_NHL == nullptr)
		{
			m_NHL = new QuadTreeNode(m_XC, m_Bounds.m_LY, m_Bounds.m_HX, m_YC, m_Depth-1);
		}
		m_NHL->AddPoint(p);
	}
	else if (NormRect::RectContainsPoint(m_XC, m_YC, m_Bounds.m_HX, m_Bounds.m_HY, p.m_NX, p.m_NY))
	{
		if(m_NHH == nullptr)
		{
			m_NHH = new QuadTreeNode(m_XC, m_YC, m_Bounds.m_HX, m_Bounds.m_HY, m_Depth-1);
		}
		m_NHH->AddPoint(p);
	}
}
