...
#ifdef TRIM_BOUNDS
		// TODO: ushort would be enough if we stored it as a delta? Or char if we stored it as a fraction?
		UInt24					minPos;				// Trim the min inwards - in the split axis
		UInt24					maxPos;				// Trim the max inwards - in the split axis
#endif
...
//		int RIsRank : 1;		// Flag to say RChild is an actual data point (A leaf) rather than a tree node
	...
	static bool XCompare(const RankedDataPoint & a, const RankedDataPoint & b)
	{ 
		return a.x < b.x; 
	}

	static bool YCompare(const RankedDataPoint & a, const RankedDataPoint & b)
	{ 
		return a.y < b.y; 
	}

	int MakeTreeRecurse(TreeNode *thisNode, const int thisNodeIndex, RankedDataPoint * const data, int firstPoint, int noofPoints, const bool doXAxisDivision)
	{
		int nextNodeIndex = thisNodeIndex + 1;
		...
		RankedDataPoint *first = data + firstPoint;
		
		if (doXAxisDivision)
		{
			std::pair<RankedDataPoint *, RankedDataPoint *> minMax = std::minmax_element(first, first + noofPoints, &XCompare);
			thisNode->minPos = minMax.first->x;
			thisNode->maxPos = minMax.second->x;
		}
		else
		{
			std::pair<RankedDataPoint *, RankedDataPoint *> minMax = std::minmax_element(first, first + noofPoints, &YCompare);
			thisNode->minPos = minMax.first->y;
			thisNode->maxPos = minMax.second->y;
		}

		if (noofPoints >= MinimumSizeForSplit)
		{
			int noofLowNodePoints = (noofPoints + 1) / 2;
			RankedDataPoint *firstLowPoint = first;
			RankedDataPoint *firstHighPoint = first + noofLowNodePoints;

			// Sort this node's points according to x or y position
			if (doXAxisDivision)
			{
				std::nth_element(firstLowPoint, firstHighPoint, firstLowPoint + noofPoints, XCompare);
			}
			else
			{
				std::nth_element(firstLowPoint, firstHighPoint, firstLowPoint + noofPoints, YCompare);
			}
	...
	void MakeTree()
	{
		// TODO: Shelve the TreeNodes that need to be further tested, not the individual points.
		// Then we'll be finding all the points that don't need bounds tests first, and we'll need to do far fewer bounds tests at the end as we'll already have a good set of low ranks

		// TODO: Use a fine grid to short-cut the first N KDTree node tests to find the one that encloses a given grid square - will need to include the bounds of that TreeNode in the grid
		//				Can make it a precise bounding box rather than the larger one that gets passed down the tree recursion

		// TODO: Trim each KDTree node along one axis, either +ve or negative? at each layer.

		// TODO: Every x coordinate is unique, so we could look up from x coordinate to kdtree node. Each x coordinate covered by the test area is a unique point... hmmm
		// TODO: Perhaps just pick the smallest axis of the test bounding box, and iterate through the x/y points that it covers, testing only the other coordinate

		// This changes the order of the array, and then we sort it back at the end
		RankedDataPoint *dataNonConst = const_cast<RankedDataPoint *>(m_dataPointsWithRank);
...
//#define SHELVE_UNTESTED_RESULTS
	...
	int32_t SearchRecurse(SearchParams &params, const TreeNode *thisNode, RectType thisNodeBounds, int firstPoint, int noofChildPoints, const bool doXAxisDivision, int32_t foundCount)
	{
		...
		// Update the bounds according to the more precise values stored in this node
		// TODO: This didn't save very much - test again once other changes have been made and perhaps use the memory for something else
		if (doXAxisDivision)
		{
			thisNodeBounds.m_lx = thisNode->minPos;
			thisNodeBounds.m_hx = thisNode->maxPos;
		}
		else
		{
			thisNodeBounds.m_ly = thisNode->minPos;
			thisNodeBounds.m_hy = thisNode->maxPos;
		}
		
		// If this node is completely within the test region, just add all its points
		if (!params.testBounds->Overlaps(thisNodeBounds))
		{
			return foundCount;
		}
		...
		if (noofChildPoints < MinimumSizeForSplit)
		{
			...
			for (int child = 0; child < noofChildPoints; ++child, ++treeRankIter)
			{
				...
				if (foundCount >= params.desiredCount)
				{
					...
					{
						// Break here as they are sorted by rank
						break;
					}
				}
	...
	{
		// TODO: Try out multithreading? Use spin-locks for speed?

		// TODO: now the results are initialised to int_max, remove all checks for "is the list full, if so compare to highest rank on list"
		InitResults(m_resultBuffer, count);
