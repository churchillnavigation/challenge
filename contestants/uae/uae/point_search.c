...
void dfs(int s) {
	int depth, flag;
	float lx, ly, hx, hy;
	if (square[s].end - square[s].begin == 0) return;
	...
	if (flag) {
		...
		int ch, x;
		ch = (s - square0[depth]) * WIDTH2 + square0[depth + 1];
		x = ch + WIDTH2;
		for (; ch < x; ch++)
			dfs(ch);
	}
...
{
	const Point *p;
	int i, NodeSize, SquareSize, RankSize;
	...
	SquareSize = m * sizeof(struct Square);
	...
	// Malloc
	MemBegin = malloc(RankSize + NodeSize + SquareSize);
