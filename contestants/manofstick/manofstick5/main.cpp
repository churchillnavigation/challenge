...
struct Surface
{
	...
	Surface(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level);
	...
	: Surface
{
	...
	Surface0(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
		: Surface(sortHelp, coordinates, points, ordering, isVertical, creators, level) 
	{
	...
	: Surface
{
	...
	Surface1(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
		: Surface(sortHelp, coordinates, points, ordering, isVertical, creators, level)
	{
	...
	: Surface
{
	...
	SurfaceUChar(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
		: Surface(sortHelp, coordinates, points, ordering, isVertical, creators, level)
	{
	...
	: Surface
{
	...
	SurfaceX(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
		: Surface(sortHelp, coordinates, points, ordering, isVertical, creators, level)
	{
...
SurfacePtr surface_factory(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
{
	if (level == 0)
		return SurfacePtr(new Surface0(sortHelp, coordinates, points, ordering, isVertical, creators, level));
	else if (level == 1)
		return SurfacePtr(new Surface1(sortHelp, coordinates, points, ordering, isVertical, creators, level));
	else if (level < 10)
		return SurfacePtr(new SurfaceUChar(sortHelp, coordinates, points, ordering, isVertical, creators, level));
	else
		return SurfacePtr(new SurfaceX(sortHelp, coordinates, points, ordering, isVertical, creators, level));
}

Surface::Surface(SortHelp& sortHelp, Coordinates& coordinates, shared_ptr<Ranks> points, RanksBy ordering, bool isVertical, VoidFunctions& creators, int level)
{
	...
	auto jobs = shared_ptr<VoidFunctions>(new VoidFunctions());
	if (!isVertical)
	{
		auto addLeft = [&, by_x, midPoint, level](){
			auto count = midPoint;
			auto points = shared_ptr<Ranks>(new Ranks(count));
			for (auto i = 0; i < count; ++i)
				(*points)[i] = (*by_x)[i];

			_left = surface_factory(sortHelp, coordinates, points, RanksBy::ByX, false, creators, level + 1);
		};
		jobs->push_back(addLeft);

		auto addRight = [&, by_x, pointCount, midPoint, level](){
			auto count = pointCount - midPoint;
			auto points = shared_ptr<Ranks>(new Ranks(count));
			for (auto i = 0; i < count; ++i)
				(*points)[i] = (*by_x)[midPoint+i];

			_right = surface_factory(sortHelp, coordinates, points, RanksBy::ByX, false, creators, level + 1);
		};
		jobs->push_back(addRight);
	}

	auto addBottom = [&, by_y, midPoint, level](){
		auto count = midPoint;
		auto points = shared_ptr<Ranks>(new Ranks(count));
		for (auto i = 0; i < count; ++i)
			(*points)[i] = (*by_y)[i];

		_bottom = surface_factory(sortHelp, coordinates, points, RanksBy::ByY, true, creators, level + 1);
	};
	jobs->push_back(addBottom);

	auto addTop = [&, by_y, pointCount, midPoint, level](){
		auto count = pointCount - midPoint;
		auto points = shared_ptr<Ranks>(new Ranks(count));
		for (auto i = 0; i < count; ++i)
			(*points)[i] = (*by_y)[midPoint + i];

		_top = surface_factory(sortHelp, coordinates, points, RanksBy::ByY, true, creators, level + 1);
	};
	...
	auto create = [jobs](){
		...
		for (auto& job : *jobs)
			tasks.push_back(create_task(job));

		auto wait_all = concurrency::when_all(tasks.begin(), tasks.end());
		wait_all.wait();
	};
...
struct SearchContext
{
	...
	SearchContext(const Point* points_begin, const Point* points_end)
	{
		...
		_topLevel = surface_factory(sort_help, _mappings.rank_idx_to_coordinates, ranks, RanksBy::ByRank, false, creators, 0);

		auto max_time_allowed = 29500;
		auto max_index_before_memory_exceeded = 1000;

		for (auto i = 0; !creators.empty() && i < max_index_before_memory_exceeded; ++i)
		{
			auto duration = chrono::system_clock::now() - start_time;
			if (chrono::duration_cast<chrono::milliseconds>(duration).count() > 29500)
				break;
