	...
	: Surface
{
	...
	PointIterator* get_iterator(const Rect& rect)
	{
		...
		auto checks = (RectChecks)(
			(area_rect.lx <= rect.lx) * 0x1 +
			(area_rect.hx >= rect.hx) * 0x2 +
			(area_rect.ly <= rect.ly) * 0x4 +
			(area_rect.hy >= rect.hy) * 0x8);

		simple_iterator.reset(rect, checks);
	...
	: Surface
{
	...
	std::shared_ptr<RanksIterator> all_iterator;
	SimpleRankIterator simple_iterator;
	CompositeIterator composite_iterator;

	Rect lower_area_rect;
	Rect upper_area_rect;
	...
	CompositeSurface(const Mappings& mappings, Ranks& points, std::shared_ptr<RanksIterator> all_iterator, SurfacePtr lower, SurfacePtr upper, int depth)
		: Surface(mappings, points, depth)
		, all_iterator(all_iterator)
		, simple_iterator(mappings.coordinates, all_iterator)
		, lower(lower)
		, upper(upper)
	{
		lower_area_rect = lower->area_rect;
		upper_area_rect = upper->area_rect;
	}
};
	...
	: CompositeSurface
{
	...
	PointIterator* get_iterator(const Rect& rect)
	{
		if (rect.hx <= lower_area_rect.hx)
		{
			if (rect.hx < area_rect.lx)
				return &(null_iterator);
			return lower->get_iterator(rect);
		}
		if (rect.lx >= upper_area_rect.lx)
		{
			if (rect.lx > area_rect.hx)
				return &(null_iterator);
			return upper->get_iterator(rect);
		}
		if (rect.ly > area_rect.hy || rect.hy < area_rect.ly)
			return &(null_iterator);

		auto lower_common_area = (std::min(rect.hx, lower_area_rect.hx) - std::max(rect.lx, lower_area_rect.lx)) * ((std::min(rect.hy, lower_area_rect.hy) - std::max(rect.ly, lower_area_rect.ly)));
		if (lower_common_area > lower->cache_quarter_area_spanned)
		{
			auto upper_common_area = (std::min(rect.hx, upper_area_rect.hx) - std::max(rect.lx, upper_area_rect.lx)) * ((std::min(rect.hy, upper_area_rect.hy) - std::max(rect.ly, upper_area_rect.ly)));
	...
	: CompositeSurface
{
	...
	PointIterator* get_iterator(const Rect& rect)
	{
		if (rect.hy <= lower_area_rect.hy)
		{
			if (rect.hy < area_rect.ly)
				return &(null_iterator);
			return lower->get_iterator(rect);
		}
		if (rect.ly >= upper_area_rect.ly)
		{
			if (rect.ly > area_rect.hy)
				return &(null_iterator);
			return upper->get_iterator(rect);
		}
		if (rect.lx > area_rect.hx || rect.hx < area_rect.lx)
			return &(null_iterator);
...
SurfacePtr surface_factory(const Mappings& mappings, Ranks& points, int depth, bool x_tainted, bool y_tainted)
{
	...
	if (!x_tainted && (y_tainted || depth % 2 == FirstDirection))
	{
