...
struct BitSet
{
	struct iterator
	{
		...
		iterator(const std::vector<uint64_t>& data)
			: _data(data)
		{
			_index = 0;
			_bit = 1;
		...
		iterator(const std::vector<uint64_t>& data, int index, uint64_t bit)
			: _data(data)
			, _index(index)
			, _bit(bit)
		{}
	...
	struct values_iterator {
		...
		values_iterator(std::vector<uint64_t>& data, int end_index, uint64_t end_bit)
			: _data(data)
		...
		values_iterator(const values_iterator& rhs)
			: _data(rhs._data)
			, _index(rhs._index)
			, _bit(rhs._bit)
			, _bit_value(rhs._bit_value)
			, _end_index(rhs._end_index)
			, _end_bit(rhs._end_bit)
		{}

		values_iterator& operator=(const values_iterator rhs)
		{
			this->_data = rhs._data;
			this->_index = rhs._index;
			this->_bit = rhs._bit;
			this->_bit_value = rhs._bit_value;
			this->_end_index = rhs._end_index;
			this->_end_bit = rhs._end_bit;
			return *this;
		}
	...
	BitSet(uint32_t count, bool set=false)
		: _count(count)
		, _data(count / 64 + (count % 64 == 0 ? 0 : 1), set ? std::numeric_limits<uint64_t>::max() : uint64_t(0))
