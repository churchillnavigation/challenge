...
SearchContext::SearchContext(int dataCount, const Point* points_begin) {
    ...
    if (dataCount > 105000) {
        pPsL10 = new cPointSearch_t<9, MAX_SEARCH_POINTS>(dataCount, points_begin);
        find = &SearchContext::useL10;
    }
    else if (dataCount > 20500) {
        pPsL9 = new cPointSearch_t<8, MAX_SEARCH_POINTS>(dataCount, points_begin);
        find = &SearchContext::useL9;
    }
    else if (dataCount > 1000) {
        pPsL8 = new cPointSearch_t<7, MAX_SEARCH_POINTS>(dataCount, points_begin);
        find = &SearchContext::useL8;
    }
    else if( dataCount > 30 ) {
        pPsL7 = new cPointSearch_t<6, MAX_SEARCH_POINTS>(dataCount, points_begin);
...
SearchContext::~SearchContext() {
    ...
    if (pPs) {
        ...
        for (int32_t ii = 0; ii < PROFILE_COUNTERS; ii++) {
            std::cout << "countMerge[" << ii << "]  = " << pPs->countMerge[ii] << std::endl;
        }
        std::cout << "countResursive = " << pPs->countResursive << std::endl;
    }
    if (pPsL10) {
        std::cout << std::endl;
        std::cout << "!!!! PROFILE: " << std::endl;
        for (int32_t ii = 0; ii < PROFILE_COUNTERS; ii++) {
            std::cout << "countMerge[" << ii << "]  = " << pPsL10->countMerge[ii] << std::endl;
        }
        std::cout << "countResursive = " << pPsL10->countResursive << std::endl;
    }
#endif
